...
public class Vocabulary : MonoBehaviour
{
    ...
    public void btn_On_play_voice()
    {
        ...
        string nameFileAudio = this.data_v_cur.index_l+""+this.data_v_cur.index_week+""+this.data_v_cur.index_v_in_week+"a.mp3";
        if (app.carrot.get_tool().check_file_exist(nameFileAudio))
        {
            Debug.Log("Play");
            this.app.texttospeech.Play(nameFileAudio);
        }
        else
        {
            this.app.texttospeech.Speak(this.s_vocabulary,nameFileAudio);
        }
